...
public class AddTemperatureEntriesCommandTest
{
    private readonly IMapper _mapper = Substitute.For<IMapper>();
    private readonly IUnitOfWork _unitOfWork = Substitute.For<IUnitOfWork>();
    private readonly TemperatureFeatureTestObjects _testObjects = new();

    private readonly AddTemperatureEntriesHandler _sut;

    public AddTemperatureEntriesCommandTest()
    {
        _sut = new AddTemperatureEntriesHandler(_unitOfWork, _mapper);
    }
    ...
    public async Task Handle_ShouldReturnNewEntries_WhenEntriesArePresent()
    {
        ...
        //Act
        var result = await _sut.Handle(new AddTemperatureEntriesCommand(_testObjects.TemperatureEntryDtos), CancellationToken.None);

        //Assert
        result.Should().NotBeNull();
        result.Should().BeEquivalentTo(_testObjects.TemperatureEntryDtos);
        await repo.Received(1).AddRangeAsync(Arg.Is<List<TemperatureEntry>>(entries
