...
public class ContinueFeatherDeviceCommandTest
{
    ...
    private readonly ContinueFeatherDeviceHandler _sut;
    private readonly FeatherDeviceFeatureTestObjects _testObjects = new();

    public ContinueFeatherDeviceCommandTest()
    ...
    public async Task Handle_ShouldCallWriteAsync_WhenDevicePresent()
    {
        ...
        //Act
        await _sut.Handle(new ContinueFeatherDeviceCommand(_testObjects.FeatherDeviceDto), CancellationToken.None);

        //Assert
        await _deviceService.Received(1).WriteDataAsync(Arg.Is(_testObjects.FeatherDeviceDto));
    }
}