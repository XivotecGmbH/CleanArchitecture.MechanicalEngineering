...
public class StartFeatherDeviceCommandTest
{
    ...
    private readonly StartFeatherDeviceHandler _sut;
    private readonly FeatherDeviceFeatureTestObjects _testObjects = new();

    public StartFeatherDeviceCommandTest()
    ...
    public async Task Handle_ShouldCallWriteAsync_WhenDevicePresent()
    {
        ...
        dtoWithAction.Action = FeatherDeviceActionsDto.Start;
        _deviceUnitOfWork.GetDeviceService<FeatherDeviceDto>().Returns(_deviceService);

        //Act
        await _sut.Handle(new StartFeatherDeviceCommand(_testObjects.FeatherDeviceDto), CancellationToken.None);
