...
public class InitializeFeatherDeviceCommandTest
{
    ...
    private readonly InitializeFeatherDeviceHandler _sut;
    private readonly FeatherDeviceFeatureTestObjects _testObjects = new();

    public InitializeFeatherDeviceCommandTest()
        => _sut = new(_deviceUnitOfWork);

    [Fact]
    public async Task Handle_ShouldCallInitializeAsync_WhenDevicePresent()
    {
        //Arrange
        _deviceUnitOfWork.GetDeviceService<FeatherDeviceDto>().Returns(_deviceService);

        //Act
        await _sut.Handle(new InitializeFeatherDeviceCommand(_testObjects.FeatherDeviceDto), CancellationToken.None);

        //Assert
        await _deviceService.Received(1).InitializeAsync(Arg.Is(_testObjects.FeatherDeviceDto));
    }
}