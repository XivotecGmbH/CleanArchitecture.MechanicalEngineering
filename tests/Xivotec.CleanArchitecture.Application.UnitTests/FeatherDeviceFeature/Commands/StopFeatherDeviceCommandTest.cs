...
public class StopFeatherDeviceCommandTest
{
    ...
    private readonly StopFeatherDeviceHandler _sut;
    private readonly FeatherDeviceFeatureTestObjects _testObjects = new();

    public StopFeatherDeviceCommandTest()
    ...
    public async Task Handle_ShouldCallWriteAsync_WhenDevicePresent()
    {
        ...
        dtoWithAction.Action = FeatherDeviceActionsDto.Stop;
        _deviceUnitOfWork.GetDeviceService<FeatherDeviceDto>().Returns(_deviceService);

        //Act
        await _sut.Handle(new StopFeatherDeviceCommand(_testObjects.FeatherDeviceDto), CancellationToken.None);
