...
public class DisconnectFeatherDeviceCommandTest
{
    ...
    private readonly DisconnectFeatherDeviceHandler _sut;
    private readonly FeatherDeviceFeatureTestObjects _testObjects = new();

    public DisconnectFeatherDeviceCommandTest()
        => _sut = new(_deviceUnitOfWork, _mediator);

    [Fact]
    public async Task Handle_ShouldCallDisconnectAsync_WhenDevicePresent()
    {
        //Arrange
        _deviceUnitOfWork.GetDeviceService<FeatherDeviceDto>().Returns(_deviceService);

        //Act
        await _sut.Handle(new DisconnectFeatherDeviceCommand(_testObjects.FeatherDeviceDto), CancellationToken.None);

        //Assert
        await _deviceService.Received(1).DisconnectAsync(Arg.Is(_testObjects.FeatherDeviceDto));
    }
}